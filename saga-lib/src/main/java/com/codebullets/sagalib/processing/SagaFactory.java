...
public class SagaFactory {
    ...
    private final Multimap<Class, Class<? extends Saga>> messagesHandledByExistingSagas = HashMultimap.create();
     ...
     */
    @Inject
    public SagaFactory(SagaAnalyzer sagaAnalyzer, SagaProviderFactory providerFactory) {
     ...
     */
    public Collection<Saga> create(Object message) {
        ...
        Collection <Class<? extends Saga>> existingSagaTypes = messagesHandledByExistingSagas.get(message.getClass());
     ...
     */
    private Saga startNewSaga(Class<? extends Saga> sagaToStart) {
        ...
        for (Map.Entry<Class<? extends Saga>, SagaHandlersMap> entry : handlersMap.entrySet()) {
            ...
            for (MessageHandler handler : sagaHandlers) {
                ...
                if (handler.getStartsSaga()) {
                    ...
                    messagesHandledByExistingSagas.put(handler.getMessageType(), sagaClass);
                }
            }
        }
    }
